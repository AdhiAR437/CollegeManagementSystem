...
function handleRegistration($conn)
{
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $query = "INSERT INTO student (fname, lname, email, phoneno, password, registration_number, class_id) 
        VALUES ('$firstname', '$lastname', '$email', '$phone', '$password', '$rollNumber', '$class_id')";

        try{if (mysqli_query($conn, $query)) {
            // Data insertion successful
            echo "<script>alert('Registration successful.'); window.location.href = 'StudentSignUp.php';</script>";
        } }
        catch(mysqli_sql_exception $test){}
            // Error occurred while inserting data
            //echo '<script>alert("Error: ' . mysqli_error($conn) . '");</script>';
            echo '<script>alert("The registration number or email id provided is already registered please use different one and try again.");</script>';
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="card mb-4">
                <div class="card-body">
                    <div class="card-text">
                        <p class="fs-4 fw-medium">Student Registration Form</p>
                    </div>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" onsubmit="return validateForm()">
                        ...
                        <div class="row mb-3">
                            <div class="col-6">
                                <label for="RollNumber" class="form-label">Registration Number</label>
                                <input type="text" id="RollNumber" class="form-label form-control border-dark dropdown-toggle" style="--bs-border-opacity: .5;" name="rollnumber" required>
                            </div>
                       
                            <div class="col-6">
                                <label for="ClassSelect" class="form-label">Select Class</label>
                                <select class="form-select form-control border-dark" style="--bs-border-opacity: .5;" id="ClassSelect" name="class_id" required>
                                    <option value="" selected disabled>Select a Class</option>
                                    <?php
                                        // Fetch class names and class IDs from the classes table
